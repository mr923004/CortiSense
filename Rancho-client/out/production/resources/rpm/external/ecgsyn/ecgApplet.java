...
/************************************************************************/
/*                                                                      */
/* rpm.external.ecgsyn.ecgApplet.java                                                       */
/*                                                                      */
/* Copyright (C) 2003 Mauricio Villarroel                               */
/*  (mauricio DOT vllarroel AT estudiantes DOT ucb DOT edu DOT bo)      */
/*                                                                      */
/* rpm.external.ecgsyn.ecgApplet.java and all its components are free software; you can     */
...
/**
 * External ECGSYN implementation (McSharry et al., 2003).
 * Generates synthetic ECG waveforms using a dynamical model.
 * This file is taken from the PhysioNet ECGSYN Java distribution and kept
 * unchanged except for the package name, so that we can call it from our
 * EcgsynGenerator adapter.
 *
 * Reference:
 * McSharry PE, Clifford GD, Tarassenko L, Smith LA.
 * "A dynamical model for generating synthetic electrocardiogram signals."
 * IEEE Trans Biomed Eng. 2003;50(3):289–94.
 */
...
public class ecgApplet extends javax.swing.JApplet implements AdjustmentListener{
    ...
    /** Initializes the applet rpm.external.ecgsyn.ecgApplet */
    public void init() {
        initComponents();
        initWindow();
    }
...
//                ecgFrame.repaint();        
    ...
    */
    ...
    // ============================================================
    // Helper methods for programmatic use from the RPM project
    // ============================================================

    public void setHeartRateMean(double heartRateMeanBpm) {
        this.hrmean = heartRateMeanBpm;
        if (txtHrmean != null) {
            txtHrmean.setText(Double.toString(heartRateMeanBpm));
        }
    }

    public void setNumberOfBeats(int numberOfBeats) {
        this.N = numberOfBeats;
        if (txtN != null) {
            txtN.setText(Integer.toString(numberOfBeats));
        }
    }

    public void setSamplingFrequencyHz(int samplingFrequencyHz) {
        this.sfecg = samplingFrequencyHz;
        if (txtSfecg != null) {
            txtSfecg.setText(Integer.toString(samplingFrequencyHz));
        }
    }

    public void setNoiseAmplitude(double noiseMillivolts) {
        this.Anoise = noiseMillivolts;
        if (txtAnoise != null) {
            txtAnoise.setText(Double.toString(noiseMillivolts));
        }
    }

    /**
     * Exposes the internal ECG data table so that other code
     * can read times and voltages after ecgFunction() has run.
     *
     * Column 0: time in seconds
     * Column 1: voltage in mV
     * Column 2: peak label (int)
     */
    public javax.swing.table.DefaultTableModel getTableValuesModel() {
        return tableValuesModel;
    }
